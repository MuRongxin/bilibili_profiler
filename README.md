...
- **交互式Web报告**：`python web.py` 启动本地服务（127.0.0.1:8000），多视频浏览 + 概览图表 + 用户画像 + 全量弹幕浏览器 + 问题弹幕榜；五标签页含完整报告（原静态报告纵向布局）；run.py/quick_test.py 分析完毕自动启动并打开（WEB_AUTOSTART 可关）
- **手动弹幕分析**：Web 弹幕浏览器勾选发送者 → 后台强制分析（UID解析+采集+画像+LLM深掘），进度轮询，完成自动跳转完整报告
